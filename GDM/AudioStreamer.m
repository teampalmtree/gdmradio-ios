...
//  GDM
//
//  Created by Daniel Farmer on 1/12/14.
...
//
static void ASPropertyListenerProc
(
void *                      inClientData,
...
)
{
	...
	AudioStreamer* streamer = (__bridge AudioStreamer *)inClientData; //TODO verify that this auto-change works (repeated mulitiple times)
	[streamer
...
//
static void ASAudioQueueIsRunningCallback
(
void*                   inUserData,
AudioQueueRef           inAQ,
...
//
static void ASAudioSessionInterruptionListener
(
__unused void*  inClientData,
...
// isPlaying
//
// returns YES if the audio is currently playing.
//
- (BOOL)isPlaying
{
	if (state == AS_PLAYING)
...
// isPaused
//
// returns YES if the audio is currently paused.
//
- (BOOL)isPaused
{
	if (state == AS_PAUSED)
	...
	@synchronized(self)
	{
		...
		if (fileLength > 0 && seekByteOffset > 0)
		{
			CFHTTPMessageSetHeaderFieldValue(message, CFSTR("Range"),
                                             (CFStringRef)CFBridgingRetain([NSString stringWithFormat:@"bytes=%ld-%ld", (long)seekByteOffset, (long)fileLength]));
			discontinuous = YES;
		}
